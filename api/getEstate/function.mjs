  ...
  handler: async (request, context) => {
    ...
    const rows = await mapPool(tenants, CONCURRENCY, async (t) => {
      try {
        ...
        return { tenant: t.name || t.id, tenantId: t.id, verdict: s.verdict, reportOnlyPolicies: s.reportOnlyPolicies, usersBlocked: s.usersBlocked, wouldBeBlockedSignIns: s.wouldBeBlockedSignIns, legacyAuthBlockedSignIns: s.legacyAuthBlockedSignIns, signInsAnalyzed: s.signInsAnalyzed, error: null };
      } catch (err) {
